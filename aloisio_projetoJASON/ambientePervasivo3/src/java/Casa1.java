...
public class Casa1 extends Environment {
	...
	// definindo a peca.... vem do servidor .... quarto, sala, escritorio
	...
	public boolean executeAction(String agName, Structure action) {
		...
		if (agName.equals("peca1_climatizacao") && action.getFunctor().contains("configurarTemperatura")) {
			System.out.println("[AMBIENTE FISICO] o arcondicionado, via rele de acionamento, representado por " + agName
		...
		if (agName.equals("peca1_iluminacao") && action.getFunctor().contains("configurarIluminacao")) {
			System.out.println("[AMBIENTE FISICO] o sistema de iluminacao, via rele de acionamento, representado por "
					+ agName + " esta configurando a iluminacao como " + iluminacao);
			quantidadeAcoesPeca1++;

			// acionar o rele desta peca
		...
		if (agName.equals("peca2_iluminacao") && action.getFunctor().contains("configurarIluminacao")) {
			System.out.println("[AMBIENTE FISICO] o sistema de iluminacao, via rele de acionamento, representado por "
					+ agName + " este configurando a iluminacao como " + iluminacao);
			quantidadeAcoesPeca2++;
			...
			try {
				...
				case 0:
					...
					if (!usuario.equals("erro")) {
						...
						// System.out.println("[AMBIENTE FISICO] um usuario vai entrar na peca");
					}
					break;
				case 1:
					...
					if (!usuario.equals("erro")) {
						...
						System.out.println("[AMBIENTE FISICO] o usuario " + extrairUsuario(usuario)
								+ " saiu da peca, mas suas configura��es permaneceram....");		
					}
					break;
				}
			} catch (Exception e) {
